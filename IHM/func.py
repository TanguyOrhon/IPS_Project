...
class Func:
    ...
    def animate(self, canvas, ui, i=0):
        ...
        if (self.stm.ADC_Value):
            ...
            self.weightInt.set((self.yval-self.V0)*125)
            self.degreInt.set((float(self.stm.angleServomoteur)-1000-self.V02)/11)
            print(float(self.stm.angleServomoteur)-self.V02)
    ...
    def button(self, ui):
        ...
        testButton = Button(ui, text="Reset", font=("Arial", 12), background="seagreen", fg="black", bd=2,
                            relief=RAISED,
                            command=self.onPushTest)
        testButton.place(x=1075, y=540, width=80,height=40)

    def onPushTest(self):
        self.V0 = float(self.stm.ADC_Value)
        self.V02= float(self.stm.angleServomoteur) - 1000